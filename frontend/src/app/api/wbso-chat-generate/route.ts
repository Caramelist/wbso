...
const POSSIBLE_FIREBASE_URLS = [
  ...
  'https://generateWBSOApplication-wbso-automation-platform-f8e9e.cloudfunctions.net'
].filter(Boolean);
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Generate Application API Route called with body:', JSON.stringify(body, null, 2));
    ...
    for (const baseUrl of POSSIBLE_FIREBASE_URLS) {
      try {
        const functionUrl = `${baseUrl}/generateWBSOApplication`;
    ...
    console.error('Generate Application API Route error:', error);
    return NextResponse.json(
      { 
        success: false, 
